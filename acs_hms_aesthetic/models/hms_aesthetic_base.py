    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _("New")) == _("New"):
                ...
                vals['name'] = self.env['ir.sequence'].with_company(vals.get('company_id')).next_by_code('acs.aesthetic.face.mapping', sequence_date=seq_date) or _("New")
...
class AcsBodyEvolution(models.Model):
    ...
    def _get_last_evolution(self):
        for rec in self:
            ...
            patient_id = rec.patient_id.id if rec.patient_id.id else self.env.context.get('acs_patient_id',False)
            last_evolution_id = self.search([('id','!=',rec._origin.id),('patient_id','=',patient_id),('date','<=',rec.date)], limit=1)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _("New")) == _("New"):
                ...
                if vals.get('date'):
                    seq_date = fields.Datetime.context_timestamp(self, fields.Datetime.to_datetime(vals['date']))
                vals['name'] = self.env['ir.sequence'].with_company(vals.get('company_id')).next_by_code('acs.aesthetic.body.evolution', sequence_date=seq_date) or _("New")
        return super().create(vals_list)
