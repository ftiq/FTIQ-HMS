...
class Patient(models.Model):
    ...
    def create_patient_related_user(self):
        for record in self:
            ...
            if not user:
                ...
                user_portal = user
            if group_portal not in user_portal.group_ids:
                user_portal.write({'active': True, 'group_ids': [Command.link( group_portal.id)]})
            ...
            if not self.env.context.get('website_id'):
                record.sudo().send_invitation_email()
    ...
    def _create_user(self):
        ...
        return self.env['res.users'].with_context(no_reset_password=True)._create_user_from_template({
            ...
            'company_ids': [Command.set( [company_id.id])],
        })
