...
class Hospitalization(models.Model):
    ...
    def acs_hospitalization_lab_data(self, invoice_id=False):
        lab_data = []
        request_ids = self.mapped('request_ids').filtered(lambda req: req.state=='to_invoice' or req.state=='done' and not req.invoice_id)
        if request_ids:
            lab_data.append({'name': _("Laboratory Charges"), 'display_type': 'line_section'})
            ...
            for record in request_ids:
                ...
                for line in record.line_ids:
                    # Sections added in the laboratory lines will appear as subsections in the invoice
                    if line.display_type in ['line_section']:
                        ...
                        lab_data.append({
                            ...
                            'acs_laboratory_request_ids': [record.id]
                        })
                if invoice_id:
