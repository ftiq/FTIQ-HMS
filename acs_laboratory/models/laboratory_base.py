...
class AcsLabTestCategory(models.Model):
    ...
    name = fields.Char(string='Category Name', required=True, tracking=True, translate=True)
...
class LabTest(models.Model):
    ...
    name = fields.Char(string='Name', help="Test Name, eg Hemogram, Biopsy...", translate=True)
    ...
    #Check error of access in multi company
    #taxes_ids = fields.Many2many(related='product_id.taxes_id', string="Taxes", readonly=True)
    list_price = fields.Float(related='product_id.list_price', string="Price", readonly=True)
    total_amount = fields.Float(string="Total", compute="acs_compute_total_amount")
    ...
    def acs_compute_total_amount(self):
        for rec in self:
            if rec.list_price and rec.product_id.taxes_id:
                taxes_res = rec.product_id.taxes_id._filter_taxes_by_company(self.env.company).compute_all(
                    rec.list_price, currency=rec.currency_id, quantity=1, product=rec.product_id, partner=None)
                rec.total_amount = taxes_res['total_included']
            else:
                rec.total_amount = rec.list_price
...
class LabTestCritearea(models.Model):
    ...
    acs_is_python_compute = fields.Boolean(string='Is Python Compute', default=False)
    acs_code = fields.Char(string='Code')
    acs_python_code = fields.Text(string='Python Code',
        default='''
        # Available variables:
        # ----------------------
        # lines: dict containing the lab test result values, where keys are test codes.
        #        Example: {'CHOL': 200, 'TG': 150, 'HDL': 50}
        # Note: returned value has to be set in the variable 'result'.
        #
        # Example Calculation: LDL = Total Cholesterol - (VLDL + HDL)
        # ------------------------------------------------------------
        # chol = float(lines.get('CHOL', 0.0))  # Total Cholesterol
        # tg = float(lines.get('TG', 0.0))      # Triglycerides
        # hdl = float(lines.get('HDL', 0.0))    # HDL Cholesterol

        # Compute VLDL as TG / 5
        # vldl = tg / 5.0

        # Compute LDL using the Friedewald formula
        # result = chol - (vldl + hdl)
        '''
        )
