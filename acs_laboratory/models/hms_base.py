...
class AccountInvoice(models.Model):
    ...
    laboratory_request_id = fields.Many2one('acs.laboratory.request', string='Lab Request', copy=False, ondelete='restrict')
...
class AccountMoveLine(models.Model):
    ...
    acs_hms_source_type = fields.Selection(selection_add=[('procedure',), ('laboratory','Laboratory')])
    
    acs_laboratory_request_ids = fields.Many2many("acs.laboratory.request", "rel_acs_lab_request_move_line", 
                                                  "move_line_id", "laboratory_request_id", string="Laboratory")
...
class ACSHmsMixin(models.AbstractModel):
    ...
    # MKA: Once the invoice is created from the laboratory even from appointment, hospitalization, 
    ...
    def acs_prepare_invoice_line_data(self, data, inv_data={}, fiscal_position_id=False):
        ...
        if data.get('acs_hms_source_type') == 'laboratory' and data.get('acs_laboratory_request_ids'):
            res['acs_laboratory_request_ids'] = [Command.set(data.get('acs_laboratory_request_ids', [self.id]))]
        return res
