...
class PatientLabTest(models.Model):
    ...
    _inherit = ['mail.thread', 'mail.activity.mixin', 'acs.hms.mixin', 'portal.mixin', 'acs.document.mixin', 'acs.qrcode.mixin', 'product.catalog.mixin']
    ...
    def acs_python_compute_code(self):
        for lab_test in self:
            lines = {}
            for line in lab_test.critearea_ids:
                if line.acs_code is not None:
                    key = str(line.acs_code)
                    try:
                        lines[key] = float(line.result)
                    except:
                        lines[key] = 0
            eval_context = {'lines': lines}
            for line in lab_test.critearea_ids:
                if line.acs_python_code and line.acs_is_python_compute != False:
                    try:
                        eval_context['result'] = line.result
                        safe_eval(line.acs_python_code, eval_context, mode='exec', nocopy=True)
                        line.result = str(eval_context.get('result', line.result))
                    except Exception as e:
                        raise UserError(_("Error in calculation for %s: %s") % (line.acs_code or line.name, str(e)))
        return True
            ...
            for line in self.test_id.critearea_ids:
                test_lines.append(
                    Command.create({
                        ...
                        'remark': line.remark,
                        'acs_code': line.acs_code,
                        'acs_is_python_compute': line.acs_is_python_compute,
                        'acs_python_code': line.acs_python_code,
    ...
    def _update_order_line_info(self, product_id, quantity, **kwargs):
        return self.acs_generic_update_order_line_info(model='hms.consumable.line', product_id=product_id, quantity=quantity,link_field='patient_lab_test_id', extra_vals=None)
