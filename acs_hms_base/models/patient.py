...
class ACSPatient(models.Model):
    ...
    def acs_get_family_partners(self):
        for rec in self:
            ...
            rec.acs_family_partner_ids = [Command.set(family_partner_ids)]
    ...
    document_type_id = fields.Many2one('acs.document.type',string="Document Type",default=lambda self: self.env['acs.document.type'].search([('acs_is_default', '=', True)], limit=1))
    ...
    occupation = fields.Char("Occupation Old") #Deprecated in v19
    acs_occupation_id = fields.Many2one( 'acs.patient.occupation', string='Occupation', index=True)
    ...
    acs_medical_record_update = fields.Datetime(string="Medical Record Last Updated", help="Date of last update of medical record for technical use of ordering patients")
    ...
    @api.onchange('phone')
    def _onchange_phone_warning(self):
        if not self.phone:
            return
        message = ''
