        ...
        if selected_patients:
            if len(selected_patients)==1:
                ...
                if laboratory_request.patient_id.id in selected_patients:
                    ...
                    data = {'is_group_request': True, 'group_patient_ids': [Command.set( user_selected_patients)]}
                else:
                    data = {'patient_id': selected_patients[0],'is_group_request': True, 'group_patient_ids': [Command.set( selected_patients)]}
                laboratory_request.sudo().write(data)
...
class AcsPaymentPortal(payment_portal.PaymentPortal):
    ...
    @http.route(['/my/lab_requests/<model("acs.laboratory.request"):request_id>/paymentprocess'], type='jsonrpc', auth="public", website=True, sitemap=False)
    def laboratoryrequest_payment_process(self, request_id, access_token, **kwargs):
        ...
        tx_sudo = self._create_transaction(
            custom_create_values={'acs_laboratory_request_id': request_id.id}, **kwargs,
        )
    ...
    def _get_extra_payment_form_values(self, **kwargs):
        ...
        acs_laboratory_request_id = kwargs.get('acs_laboratory_request_id')
        if acs_laboratory_request_id:
            acs_laboratory_request_id = self._cast_as_int(acs_laboratory_request_id)
            order_sudo = request.env['acs.laboratory.request'].sudo().browse(acs_laboratory_request_id)
