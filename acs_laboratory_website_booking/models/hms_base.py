...
class PaymentTransaction(models.Model):
    ...
    acs_laboratory_request_id = fields.Many2one("acs.laboratory.request", string="Lab Request")

    #Update Payments directs after successful payment.
    def _post_process(self):
        res = super()._post_process()
        for tx in self.filtered(lambda t: t.operation != 'validation' and t.acs_laboratory_request_id):
            tx._acs_update_laboratory_request()
        return res

    #Update Lab request data.
    def _acs_update_laboratory_request(self):
        self.acs_laboratory_request_id.sudo().with_context(acs_online_transaction=True,default_create_stock_moves=False).create_invoice()
        if self.acs_laboratory_request_id.sudo().invoice_id.state=='draft':
            self.acs_laboratory_request_id.sudo().invoice_id.action_post()
        if self.payment_id and self.acs_laboratory_request_id.sudo().invoice_id:
            self.payment_id.invoice_ids |= self.acs_laboratory_request_id.sudo().invoice_id

        if self.acs_laboratory_request_id.sudo().state!='confirm':
            self.acs_laboratory_request_id.sudo().with_context(acs_online_transaction=True).button_accept()
        ...
        self.acs_laboratory_request_id.invoice_id._portal_ensure_token()
        self.invoice_ids = [Command.set( [self.acs_laboratory_request_id.invoice_id.id])]
