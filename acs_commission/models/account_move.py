...
class AccountMoveLine(models.Model):
    ...
    acs_commission_partner_ids = fields.Many2many("res.partner", "partner_move_line_commission_rel", "move_line_id", "partner_id", 
        string="Commission For", domain="[('provide_commission','=',True)]")
...
class AccountMove(models.Model):
    ...
    commission_partner_ids = fields.Many2many("res.partner", "partner_invoice_commission_rel", "invoice_id", "partner_id", 
        string="Commission For", domain="[('provide_commission','=',True)]")
    ...
    def onchange_acs_invoice_user(self):
        if self.invoice_user_id and self.invoice_user_id.provide_commission:
            self.commission_partner_ids = [Command.link( self.invoice_user_id.partner_id.id)]
    ...
    def create_partner_commission(self, partner, amount, commission_on, invoice_line=[]):
        ...
        invoice_line_data = [Command.link( line_id) for line_id in invoice_line.ids] if invoice_line else []
        if commission_line:
            if amount:
