...
class OtBooking(models.Model):
    ...
    end_date = fields.Datetime('End Date', default=_get_end_date, copy=False)
    hospitalization_id = fields.Many2one('acs.hospitalization', string='Hospitalization')
    surgery_template_id = fields.Many2one('hms.surgery.template', string='Surgery Template')
    surgery_id = fields.Many2one('hms.surgery', string='Surgery', copy=False)
    ...
    @api.constrains('start_date','end_date','ot_id','state')
    def _check_booking_conflict(self):
        for rec in self:
            if rec.ot_id and rec.ot_id.avoid_booking_overlapping:
                ...
                if other_bookings:
                    raise ValidationError((_("%s is already booked for selected time")))
    ...
    def button_confirm(self):
        ...
        self.name = self.env['ir.sequence'].next_by_code('acs.ot.booking') or 'New'
