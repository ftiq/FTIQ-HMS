...
class ACSPhysiotherapyExerciseGroup(models.Model):
    ...
    _code_uniq = models.Constraint(
        'unique (name)',
        "Physiotherapy Group Name must be unique",
    )
...
class ACSPhysiotherapy(models.Model):
    ...
    _inherit = ['mail.thread', 'mail.activity.mixin', 'acs.hms.mixin', 'acs.calendar.mixin']
    _order = "id desc"

    def acs_get_age(self):
        for rec in self:
            age = ''
            if rec.patient_id.birthday:
                age = _("%s years") % ((relativedelta(datetime.now(), rec.patient_id.birthday)).years)
            rec.age = age
    ...
    def _get_prescription_history(self):
        for rec in self:
            history = ""
            for prescription in self.env['prescription.order'].search([('state','not in',['draft','cancel']),('patient_id', '=', rec.patient_id.id)], limit=10):
                history += "<table class='table table-condensed'>"
                history += _("<tr><td><b>Date:</b></td><td>%s</td><td><b>Number:</b></td><td>%s</td></tr>")%(prescription.prescription_date,prescription.name)
                history += _("<tr><td><b>Diagnosis:</b></td><td>%s</td><td></td><td></td></tr>")%(', '.join(map(lambda x: (x.name), prescription.diseases_ids)))
                history += _("""<td colspan="4">
                             <table class='table table-condensed'>
                                <tr>
                                    <th name="th_no"></th>
                                    <th name="th_description" class="text-start">Rx</th>
                                    <th name="th_dose" class="text-start">Dose Qty.</th>
                                    <th name="th_total" class="text-start">Total Qty.</th>
                                </tr>
                             """)
                line_no = 1
                for line in prescription.prescription_line_ids:
                    history += _("<tr><td>%s</td><td>%s</td><td>%s %s</td><td>%s</td></tr>")%(line_no, line.name or line.product_id.name, line.dose, line.dosage_uom_id.name, line.quantity)
                    line_no += 1
                history += _("</table></td></tr>")
                history += _("<tr><td style='border-bottom: 1px solid black'><b>Findings:</b></td><td colspan='3' style='border-bottom: 1px solid black'>%s</td></tr>")%(prescription.appointment_id.notes or '')
                history += "</table>"
    ...
    def _get_physiotherapy_history(self):
        for rec in self:
            ...
            for line in self.search([('state','in',['done','to_invoice']),('patient_id', '=', rec.patient_id.id)],order='date desc', limit=10):
    ...
    def _get_rec_total(self):
        for rec in self:
            rec.visit_count = self.search_count([('state','in',['done','to_invoice']),('patient_id', '=', rec.patient_id.id)])
    ...
    @api.model
    def acs_get_default_date_stop(self):
        #odoo19: add configurable option or dynamic logic for it
        duration = 1
    ...
    patient_id = fields.Many2one('hms.patient', string='Patient',required=True, index=True,
        help='Patient Name', tracking=1)
    gender = fields.Selection(related="patient_id.gender", string="Gender", readonly=True, store=True)
    age = fields.Char(compute="acs_get_age", string='Age', readonly=True, store=True)
    code =  fields.Char(related='patient_id.code', string='Reg. No.', readonly=True)
    image_128 = fields.Binary(related='patient_id.image_128',string='Image', readonly=True)
    date = fields.Datetime(string='Date', default=fields.Datetime.now(), tracking=1)
    date_stop = fields.Datetime("End Date", tracking=1, default=acs_get_default_date_stop)
    ...
    state = fields.Selection([('draft', 'Draft'), 
        ...
        ('done', 'Done'),
        ('cancel', 'Cancel'),
        ('to_invoice', 'To Invoice'),], string='Status', readonly=True, default='draft', tracking=1)
    ...
    def write(self, values):
        ...
        fields_to_check = ['date','date_stop','physiotherapist_id','state']
        if any(f in values for f in fields_to_check):
            self.acs_calendar_event('physiotherapist_id')
        return res

    def acs_prepare_calendar_data(self):
        data = super().acs_prepare_calendar_data()
        user_id = self.physiotherapist_id
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _("New")) == _("New"):
                ...
                vals['name'] = self.env['ir.sequence'].with_company(vals.get('company_id')).next_by_code('physiotherapy.code', sequence_date=seq_date) or _("New")
