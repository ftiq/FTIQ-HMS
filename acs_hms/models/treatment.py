...
class ACSTreatment(models.Model):
    ...
    move_line_ids = fields.Many2many("account.move.line", "rel_acs_treatment_move_line", 
                                     "treatment_id", "move_line_id", string="Move Lines")
    ...
    def treatment_running(self):
        patient_disease_id = self.env['hms.patient.disease'].create({
            'patient_id': self.patient_id.id,
            'treatment_id': self.id,
            'disease_ids': [Command.set( [self.diagnosis_id.id])] if self.diagnosis_id.id else [],
            'age': self.age,
    ...
    def create_invoice(self):
        ...
        product_data = [{'product_id': product_id, 'acs_hms_source_type': 'treatment', 'acs_treatment_ids': [self.id]}]
        inv_data = {'hospital_invoice_type': 'treatment', 'acs_treatment_id': self.id}
    ...
    def action_create_procedure_invoice(self):
        ...
        for procedure in procedure_ids:
            product_data.append({
                ...
                'acs_hms_source_type': 'procedure',
                'acs_procedure_ids': [procedure.id]
            })
    ...
    def view_invoice(self):
        invoices = self.move_line_ids.mapped('move_id') + self.patient_procedure_ids.mapped('invoice_id')
