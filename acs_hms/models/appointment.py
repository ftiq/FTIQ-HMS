...
class Appointment(models.Model):
    ...
    def _acs_get_invoice_count(self):
        for rec in self:
            rec.invoice_count = len(self.acs_appointment_invoices())
    ...
    def get_procedures_to_invoice(self):
        ...
        for rec in self:
            ...
            rec.procedure_to_invoice_ids = [Command.set( procedures.ids)]
    ...
    move_line_ids = fields.Many2many("account.move.line", "rel_acs_appointment_move_line", 
                                     "appointment_id", "move_line_id", string="Move Lines")
    ...
    def update_reminder_dates(self):
        for rec in self:
            if fields.Datetime.now() < rec.date and (rec.company_id.acs_reminder_day or rec.company_id.acs_reminder_hours):
    ...
    def acs_update_patient_medical_record_date(self):
        self.sudo().mapped('patient_id').acs_medical_record_update = fields.Datetime.now()
    ...
    def create(self, vals_list):
        ...
        res.acs_update_patient_medical_record_date()
        for record in res:
    ...
    def write(self, values):
        ...
        if 'date' in values:
            ...
            self.acs_update_patient_medical_record_date()
    ...
    def acs_prepare_calendar_data(self):
        ...
        data.update({
            ...
            'start': self.date,
            'stop': self.date_to,
            'partner_ids': [Command.set( [user_id.partner_id.id])],
        })
        return data
    ...
    def acs_appointment_inv_product_data(self, with_product=True):
        product_data = []
        invoiced_products = self.env['account.move.line'].search([
            ('move_id.appointment_id', '=', self.id),
            ('move_id.state', '!=', 'cancel')
        ]).mapped('product_id.id')

        if with_product and not self.invoice_exempt:
            if self.product_id and self.product_id.id not in invoiced_products:
                product_data.append({
                    'product_id': self.product_id, 
                    'line_type': 'appointment',
                    'acs_hms_source_type': 'appointment',
                    'acs_appointment_ids': [self.id]
                })
        ...
        if self.consumable_line_ids:
            ...
            for consumable in self.consumable_line_ids:
                ...
                if consumable.acs_invoice_exempt or consumable.product_id.id in invoiced_products:
                    continue
                # Sections added in the appointment lines will appear as subsections in the invoice
                if consumable.display_type in ['line_section']:
                    consumable_lines.append({
                        'name': consumable.name,
                        'display_type': 'line_subsection',
                    })
                elif consumable.display_type in ['line_note']:
                    consumable_lines.append({
                        'name': consumable.name,
            ...
            if consumable_lines:
                product_data.append({'name': _("Consumed Product/services")})
                product_data += consumable_lines
    ...
    def acs_appointment_invoices(self):
        """
            MKA: It will return the invoice if the appointment is linked to an invoice line. 
    ...
    def view_invoice(self):
        final_invoices = self.acs_appointment_invoices()
        action = self.acs_action_view_invoice(final_invoices)
    ...
    def onchange_patient_id(self):
        if self.patient_id:
            ...
            referring_partner = self.acs_get_referring_partner(self.patient_id, referral_type='consultation')
    ...
    def action_view_treatment(self):
        ...
        action['context'] = {
            'default_appointment_ids': [Command.set(self.ids)],
            'default_patient_id': self.patient_id.id,
            'default_physician_id': self.physician_id.id,
    ...
    def action_view_patient_procedures(self):
        ...
        action['context'] = {
            'default_treatment_id': self.treatment_id and self.treatment_id.id or False,
            'default_appointment_ids': [Command.set([self.id])],
            'default_patient_id': self.patient_id.id,
            'default_physician_id': self.physician_id.id,
