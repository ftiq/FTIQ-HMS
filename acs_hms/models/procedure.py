...
class AcsPatientProcedure(models.Model):
    ...
    name = fields.Char(string="Name", tracking=1, default='New', copy=False)
    ...
    move_line_ids = fields.Many2many("account.move.line", "rel_acs_procedure_move_line", 
                                     "procedure_id", "move_line_id", string="Move Lines")
        ...
        if not self.patient_disease_id:
            ...
            if disease_history:
                disease_history.write({
                    ...
                    'procedure_ids': [Command.link( self.id)],
                    'disease_ids': [Command.link( disease_id)] if disease_id else False,
                })
            else:
                patient_disease_id = self.env['hms.patient.disease'].create({
                    ...
                    'physician_id': self.physician_id.id,
                    'diagnosed_date': self.date,
                    'procedure_ids': [Command.set( [self.id])],
                    'disease_ids': [Command.set( [disease_id])] if disease_id else False,
                })
    ...
    def view_invoice(self):
        invoices = self.move_line_ids.mapped('move_id')
