...
class ACSPrescriptionOrder(models.Model):
        ...
        for rec in self:
            ...
            for line in rec.group_id.medicament_group_line_ids:
                product_lines.append((0,0,{
                    ...
                    'active_component_ids': [Command.set( [x.id for x in line.product_id.active_component_ids])],
    ...
    def onchange_patient(self):
        if self.patient_id:
            ...
            referring_partner = self.acs_get_referring_partner(self.patient_id, referral_type='pharmacy')
...
class ACSPrescriptionLine(models.Model):
    ...
    def onchange_product(self):
        if self.product_id:
            self.active_component_ids = [Command.set( [x.id for x in self.product_id.active_component_ids])]
