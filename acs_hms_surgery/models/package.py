...
class AcsHmsPackage(models.Model):
    ...
    @api.depends('order_line.price_total', 'product_id', 'pricelist_id', 'product_id.lst_price')
    def _amount_all(self):
        ...
        for package in self:
            ...
            package.service_amount = package.company_id.currency_id.round(price)
    ...
    currency_id = fields.Many2one('res.currency', default=lambda self: self.env.company.currency_id.id, string="Currency", readonly=True, required=False)
    ...
    def acs_update_product_price(self):
        for line in self.order_line:
            line.acs_pricelist_onchange_product()
...
class AcsHmsPackageLine(models.Model):
    ...
    def product_id_change(self):
        ...
        vals = {}
        domain = {'product_uom_id': [('id', '=', self.allowed_uom_ids.ids)]}
    ...
    @api.onchange('product_id')
    def acs_pricelist_onchange_product(self):
        if self.product_id:
            price = 0.0
            if self.order_id.pricelist_id:
                price = self.product_id.with_context(
                    acs_pricelist_id=self.order_id.pricelist_id.id
