...
class MedicalVisit(models.Model):
    ...
    name = fields.Char(size=256, string='Name', tracking=True, copy=False,default='New')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _("New")) == _("New"):
                seq_date = None
                if vals.get('date_visit'):
                    seq_date = fields.Datetime.context_timestamp(self, fields.Datetime.to_datetime(vals['date_visit']))
                vals['name'] = self.env['ir.sequence'].with_company(vals.get('company_id')).next_by_code('acs.mr.visit', sequence_date=seq_date) or _("New")
        return super().create(vals_list)
...
class ACSSampleLine(models.Model):
    ...
    product_id = fields.Many2one('product.product', ondelete="restrict", string='Product')
