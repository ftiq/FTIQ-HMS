...
from odoo import api, fields, models, http, tools, _, SUPERUSER_ID, Command
...
class AcsRadiologyRequest(models.Model):
    ...
    def onchange_schedule_date(self):
        if not self.env.context.get('default_schedule_slot_id'):
    ...
    #Based on selected collection center update type
    def write(self, vals):
        if 'radiology_room_id' in vals and vals.get('radiology_room_id'):
            radiology_room_id = self.env['acs.radiology'].sudo().search([('id','=',vals.get('radiology_room_id'))])
        ...
        return super(AcsRadiologyRequest, self).write(vals)

    @api.model
    def clear_acs_radiology_request_cron(self):
    ...
    def button_cancel(self):
        res = super(AcsRadiologyRequest, self).button_cancel()
        self.schedule_slot_id = False
        return res
...
class AcsLabTest(models.Model):
    _inherit = 'acs.radiology.test'

    image_1920 = fields.Image("Image")
    allowed_online_booking = fields.Boolean("Allowed Online Booking")


class AcsRadiologyRoom(models.Model):
    _inherit = 'acs.radiology.room'

    image_1920 = fields.Image("Image")
    allowed_online_booking = fields.Boolean("Allowed Online Booking", default=True)


class PaymentTransaction(models.Model):
    _inherit = "payment.transaction"

    acs_radiology_request_id = fields.Many2one("acs.radiology.request", string="Radiology Request")

    #Update Payments directs after successful payment.
    def _post_process(self):
        res = super()._post_process()
        for tx in self.filtered(lambda t: t.operation != 'validation' and t.acs_radiology_request_id):
            tx._acs_update_radiology_request()
        return res

    #Update Radiology request data.
    def _acs_update_radiology_request(self):
        self.acs_radiology_request_id.sudo().with_context(acs_online_transaction=True,default_create_stock_moves=False).create_invoice()
        if self.acs_radiology_request_id.sudo().invoice_id.state=='draft':
            self.acs_radiology_request_id.sudo().invoice_id.action_post()
        if self.payment_id and self.acs_radiology_request_id.sudo().invoice_id:
            self.payment_id.invoice_ids |= self.acs_radiology_request_id.sudo().invoice_id
        if self.acs_radiology_request_id.sudo().state!='confirm':
            self.acs_radiology_request_id.sudo().with_context(acs_online_transaction=True).button_accept()
        ...
        self.acs_radiology_request_id.invoice_id._portal_ensure_token()
        self.invoice_ids = [Command.set( [self.acs_radiology_request_id.invoice_id.id])]
