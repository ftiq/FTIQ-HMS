...
class ACSRadiologyTestUom(models.Model):
    ...
    _code_uniq = models.Constraint(
        'unique (name)',
        "The Radiology Test code must be unique",
    )
...
class RadiologyTest(models.Model):
    ...
    name = fields.Char(string='Name', help="Test type, eg X-Ray, CT-Scan...", translate=True)
    ...
    total_amount = fields.Float(string="Total", compute="acs_compute_total_amount", store=True)
    ...
    @api.depends('list_price', 'product_id')
    def acs_compute_total_amount(self):
        for rec in self:
            if rec.list_price and rec.product_id.taxes_id:
                taxes_res = rec.product_id.taxes_id.compute_all(
