...
class AppointmentSonographyPelvis(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _("New")) == _("New"):
                ...
                vals['name'] = self.env['ir.sequence'].with_company(vals.get('company_id')).next_by_code('sonography.pelvis', sequence_date=seq_date) or _("New")
    ...
    def onchange_appointment(self):
        if self.appointment_id and self.appointment_id.physician_id:
            self.physician_id = self.appointment_id.physician_id.id
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _("New")) == _("New"):
                ...
                if vals.get('date'):
                    seq_date = fields.Datetime.context_timestamp(self, fields.Datetime.to_datetime(vals['date']))
                vals['name'] = self.env['ir.sequence'].with_company(vals.get('company_id')).next_by_code('sonography.follical', sequence_date=seq_date) or _("New")
        return super().create(vals_list)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _("New")) == _("New"):
                ...
                if vals.get('date'):
                    seq_date = fields.Datetime.context_timestamp(self, fields.Datetime.to_datetime(vals['date']))
                vals['name'] = self.env['ir.sequence'].with_company(vals.get('company_id')).next_by_code('sonography.obstetric', sequence_date=seq_date) or _("New")
        return super().create(vals_list)
