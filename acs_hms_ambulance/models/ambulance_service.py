...
class AcsAmbulanceService(models.Model):
    ...
    service_product_id = fields.Many2one('product.product', ondelete='restrict', 
        ...
        domain=[('hospital_product_type', '=', "ambulance")], default=_get_service_id)
    ...
    quantity = fields.Float('Distance')
    ...
    @api.onchange('service_product_id', 'quantity')
    def onchange_service_product(self):
        if self.service_product_id:
            self.price = self.service_product_id.lst_price * (self.quantity or 1)
    
    @api.onchange('odometer_start', 'odometer_stop')
    def onchange_odometer_value(self):
        #MKA: Earlier it was not calculating the quantity if the odometer_start is Zero.
        if self.odometer_start is not None and self.odometer_stop is not None:
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _("New")) == _("New"):
                ...
                vals['name'] = self.env['ir.sequence'].with_company(vals.get('company_id')).next_by_code('acs.ambulance.service', sequence_date=seq_date) or _("New")
    ...
    def action_create_invoice(self):
        ...
        product_data = [{'product_id': product_id, 'quantity': self.quantity}]
        inv_data = {'acs_ambulance_service_id': self.id ,'hospital_invoice_type': 'ambulance'}
