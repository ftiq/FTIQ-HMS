...
class HMSWebsite(Website):
    ...
    def user_booking_data(self, post):
        ...
        appointment_by = post.get('appointment_by','physician')
        ...
        if appointment_by=='physician':
            department = ''
        if appointment_by=='department':
            physician = ''
    ...
    def create_appointment_data(self, redirect=None, **post):
        appointment_by = post.get('appointment_by','physician')
        if appointment_by=='physician' and 'department_id' in post:
            post.pop('department_id')
        if appointment_by=='department' and 'physician_id' in post:
    ...
    def save_appointment(self, redirect=None, **post):
        env = request.env
        partner = env['res.users'].browse(request.env.uid).partner_id
...
class AcsPaymentPortal(payment_portal.PaymentPortal):
    ...
    @http.route(['/my/appointments/<model("hms.appointment"):appointment>/paymentprocess'], type='jsonrpc', auth="public", website=True, sitemap=False)
