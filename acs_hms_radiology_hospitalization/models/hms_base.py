...
class Hospitalization(models.Model):
    ...
    def acs_hospitalization_radiology_data(self, invoice_id=False):
        radiology_data = []
        request_ids = self.mapped('radiology_request_ids').filtered(lambda req: req.state=='to_invoice' or req.state=='done' and not req.invoice_id)
        if request_ids:
            radiology_data.append({'name': _("Radiology Charges"), 'display_type': 'line_section'})
            ...
            for record in request_ids:
                ...
                for line in record.line_ids:
                    # Sections added in the radiology lines will appear as subsections in the invoice
                    if line.display_type in ['line_section']:
                        ...
                        radiology_data.append({
                            ...
                            'acs_radiology_request_ids': [record.id]
                        })
                if invoice_id:
