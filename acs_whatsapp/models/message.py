...
class AcsWhatsAppMessage(models.Model):
    ...
    phone =  fields.Char(string='Destination Number', required=True)
    ...
    def acs_sanitize_phone_number(self, values):
        if values.get('phone'):
    ...
    def create(self, vals_list):
        for values in vals_list:
            ...
            values = self.acs_sanitize_phone_number(values)
    ...
    def write(self, vals):
        ...
        if 'phone' in vals:
            vals = self.acs_sanitize_phone_number(vals)
...
class ACSwhatsappMixin(models.AbstractModel):
    ...
    @api.model
    def send_whatsapp(self, message, phone, partner=False, template=False, res_model=False, res_id=False, company_id=False):
        if not company_id:
    ...
    @api.model
    def send_whatsapp_file_url(self, file_url, phone, partner=False, template=False, res_model=False, res_id=False, company_id=False):
        if not company_id:
            company_id = self.env.user.sudo().company_id.id
        record = self.env['acs.whatsapp.message'].create({
            'file_url': file_url,
    ...
    @api.model
    def send_whatsapp_file(self, filedata, file_name, phone, partner=False, template=False, res_model=False, res_id=False, company_id=False):
        if not company_id:
            company_id = self.env.user.sudo().company_id.id
        record = self.env['acs.whatsapp.message'].create({
            'file': filedata,
    ...
    def acs_whatsapp_chat_history(self, partner, phone):
        """Hook method to add logic in related module"""
        pass
