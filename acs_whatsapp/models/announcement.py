...
class WhatsappAnnouncement(models.Model):
    ...
    def onchange_template(self):
        if self.template_id:
            ...
            self.employee_ids = [Command.set( self.template_id.employee_ids.ids + self.employee_ids.ids)]
            self.partner_ids = [Command.set( self.template_id.partner_ids.ids + self.partner_ids.ids)]
    ...
    def acs_create_message(self, phone, partner=False, template=False, res_model=False, res_id=False):
        if self.message_type=='message':
            self.send_whatsapp(self.message, phone, partner, template=template, res_model=res_model, res_id=res_id, company_id=self.company_id.id)
        elif self.message_type=='file_url':
            self.send_whatsapp_file_url(self.file_url, phone, partner, template=template, res_model=res_model, res_id=res_id, company_id=self.company_id.id)
        elif self.message_type=='file':
            self.send_whatsapp_file(self.file, self.file_name, phone, partner, template=template, res_model=res_model, res_id=res_id, company_id=self.company_id.id)
    ...
    def send_message(self):
        ...
        if self.announcement_type=='contacts':
            for partner in self.partner_ids:
                if partner.phone:
                    self.acs_create_message(partner.phone, partner, template=template, res_model='res.partner', res_id=partner.id)
        else:
            ...
            for employee in employees:
                ...
                phone = partner.phone or employee.phone_phone
                if phone:
                    self.acs_create_message(phone, False, template=template, res_model='hr.employee', res_id=employee.id)
