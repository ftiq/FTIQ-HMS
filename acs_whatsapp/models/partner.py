...
class ResPartner(models.Model):
    ...
    verified_phone_whatsapp = fields.Boolean(string="Verified Whatsapp", help="The phone number is verified using the Whatsapp message")
    ...
    def whatsapp_chat_history(self):
        ...
        return self.acs_whatsapp_chat_history(self.partner_id, self.phone)
        ...
        for rec in self:
            ...
            if template_id:
                if rec.phone:
                    ...
                    msg = rendered[rec.id]
                    self.with_context(force_send=True).send_whatsapp(msg, rec.phone, False, template=template_id, res_model='res.partner', res_id=rec.id, company_id=company_id.id)
                else:
    ...
    def action_verify_otp_whatsapp(self):
        ...
        if self.generated_otp_whatsapp == self.acs_otp_whatsapp:
            self.verified_phone_whatsapp = True
        else:
    ...
    def action_acs_whatsapp_announcement(self):
        ...
        action['context'] = { 'default_partner_ids': [Command.set( [self.id])] }
        return action

    def whatsapp_chat_history(self):
        if not self.phone:
            raise UserError(_("No Mobile no linked with Record."))     
        return self.acs_whatsapp_chat_history(self, self.phone)
