...
class FacilityActivity(models.Model):
    ...
    name = fields.Char(size=256, string='Sequence',default='New')
    activity_name = fields.Char('Activity Name', required=True)
    date_activity = fields.Date('Date')
    user_id = fields.Many2one('res.users','Assigned To', help="Name of the reviewer who is Assigned the Activity", ondelete="restrict")
    reviewer_id = fields.Many2one('res.users','Reviewer', help="Name of the reviewer who is reviewing the Activity", ondelete="restrict")
    date_review = fields.Date('Review Date', readonly=True)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _("New")) == _("New"):
                ...
                vals['name'] = self.env['ir.sequence'].with_company(vals.get('company_id')).next_by_code('facility.activity', sequence_date=seq_date) or _("New")
...
class FacilityMaster(models.Model):
    ...
    reviewer_id = fields.Many2one('res.users','Reviewer', 
        help="Name of the reviewer who is reviewing the task", ondelete="restrict")
    recurring_type = fields.Selection([
        ('daily', 'Day(s)'),
        ('weekly', 'Week(s)'),
        ('monthly', 'Month(s)'),
        ('yearly', 'Year(s)'), ], default="daily", required=True, string='Recurring Period')
    recurring_interval = fields.Integer(string="Repeat Every", default=1, required=True)
    ...
    state = fields.Selection([
        ...
        ('running','Running'),
        ('done','Done'),
        ('cancel','Cancel')], 'Status', default="draft", tracking=1)
    remark = fields.Text('Remark')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _("New")) == _("New"):
                ...
                if vals.get('start_date'):
                    seq_date = fields.Datetime.context_timestamp(self, fields.Datetime.to_datetime(vals['start_date']))
                vals['name'] = self.env['ir.sequence'].with_company(vals.get('company_id')).next_by_code('facility.master', sequence_date=seq_date) or _("New")
        return super().create(vals_list)
