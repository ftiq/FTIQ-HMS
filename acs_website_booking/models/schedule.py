...
class ACSSchedule(models.Model):
    ...
    def acs_get_default_schedule_lines(self):
        return [
            ...
            Command.create( {'name': _('Monday Evening'), 'dayofweek': '0', 'hour_from': 13, 'hour_to': 17, 'day_period': 'afternoon'}),
            Command.create( {'name': _('Tuesday Morning'), 'dayofweek': '1', 'hour_from': 8, 'hour_to': 12, 'day_period': 'morning'}),
            Command.create( {'name': _('Tuesday Evening'), 'dayofweek': '1', 'hour_from': 13, 'hour_to': 17, 'day_period': 'afternoon'}),
            Command.create( {'name': _('Wednesday Morning'), 'dayofweek': '2', 'hour_from': 8, 'hour_to': 12, 'day_period': 'morning'}),
            Command.create( {'name': _('Wednesday Evening'), 'dayofweek': '2', 'hour_from': 13, 'hour_to': 17, 'day_period': 'afternoon'}),
            Command.create( {'name': _('Thursday Morning'), 'dayofweek': '3', 'hour_from': 8, 'hour_to': 12, 'day_period': 'morning'}),
            Command.create( {'name': _('Thursday Evening'), 'dayofweek': '3', 'hour_from': 13, 'hour_to': 17, 'day_period': 'afternoon'}),
            Command.create( {'name': _('Friday Morning'), 'dayofweek': '4', 'hour_from': 8, 'hour_to': 12, 'day_period': 'morning'}),
            Command.create( {'name': _('Friday Evening'), 'dayofweek': '4', 'hour_from': 13, 'hour_to': 17, 'day_period': 'afternoon'})
        ]
    ...
    def acs_get_booking_price(self):
        for rec in self:
            ...
            # if rec.department_id and rec.department_id.consultation_service_id:
            #     product = rec.department_id.consultation_service_id
            if not product:
    ...
    #ACS: Can be splitted in related module if needed.
    #To Avoid code duplication in phone api.
    def acs_get_slot_lines(self, **args):
        ...
        radiology_room_id = args.get('radiology_room_id')
        domain = [('state','=','available')]
        if date:
            domain += [('acs_slot_id.slot_date','=', date),('from_slot','>=',fields.Datetime.now())]
        else:
        ...
        if collection_center_id:
            domain += [('acs_slot_id.acs_schedule_id.collection_center_id', '=', int(collection_center_id))]
        if radiology_room_id:
            domain += [('acs_slot_id.acs_schedule_id.radiology_room_id', '=', int(radiology_room_id))]
    ...
    #To Avoid code duplication in phone api also 
    #some parameters are not generic but to avoid duplication it is kept as it is.
    def acs_get_slot_data(self, **args):
        '''Pass following parameter in args:
            physician_id: Search By dr.
            department_id: Search By Department.
            date: for specific date
            schedule_type: by schedule type
            collection_center_id: for specific collection center used in lab.
            radiology_room_id: for specific collection center used in lab.
        '''
        slot_lines = self.acs_get_slot_lines(**args)
        slot_data = []
        for slot in slot_lines:
            physician_field_exists = True if 'physician_id' in slot._fields else False
            slot_data.append({
                'date': slot.acs_slot_id.slot_date.strftime('%m/%d/%Y'),
                'name': slot.name,
                'id': slot.id,
                'physician_id': physician_field_exists and slot.physician_id.id or False,
                'physician_name': physician_field_exists and slot.physician_id and slot.physician_id.name or '',
...
class ACSscheduleSlot(models.Model):
    ...
    company_id = fields.Many2one('res.company', string='Company', related="acs_schedule_id.company_id", store=True)

    show_fee_on_booking = fields.Boolean(related="acs_schedule_id.show_fee_on_booking", string="Show Fees", store=True)
    schedule_type = fields.Selection(related="acs_schedule_id.schedule_type", string="Schedule Type", store=True)

    _slot_date_unique = models.Constraint(
        'UNIQUE(slot_date, acs_schedule_id)',
        "Booking slot must be unique!",
    )
    ...
    def acs_get_slot_line_data(self, slot, from_slot, to_slot, **kw):
        line_data = {
            ...
            'acs_slot_id': slot.id,
            'limit': kw.get('limit'),
            'user_id': kw.get('user_id') and kw.get('user_id').id or self.env.user.id,
            'schedule_type': slot.acs_schedule_id.schedule_type
        }
        if kw.get('limit'):
    ...
    def _create_slot_interval(self, slot, start_dt, hour_from, hour_to, booking_slot_time, **kw):
        ...
        start = start_dt #.date()
        #run_time = hour_from
        while (round(hour_from,2) < round(hour_to,2)):
    ...
    def acs_get_resources(self, schedule, **kw):
        ...
        if not user_ids and schedule.user_ids:
            user_ids = schedule.user_ids
        return field_name, user_ids
    ...
    def create_slot(self, slot_date, schedule, booking_slot_time=False, **kw): 
        ...
        for line in ScheduleLines.search([('acs_schedule_id','=',schedule.id),('dayofweek','=',str(weekday))]):
            field_name, resources = self.acs_get_resources(schedule, **kw)
            if resources:
                for resource in resources:
                    new_kw = kw
                    new_kw[field_name] = resource
                    self._create_slot_interval(slot, slot_date, line.hour_from, line.hour_to, booking_slot_time, **new_kw)
...
class ACSscheduleSlotLines(models.Model):
    ...
    #update in related module.
    #Check if multiple module linked it is getting updated or not.
    def get_acs_remaining_limit(self):
        for rec in self:
            rec.rem_limit = 1
    
    @api.depends('is_blocked', 'rem_limit')
    def get_acs_status(self):
        for rec in self:
            state = 'available'
            if rec.is_blocked:
                state = 'blocked'
            elif rec.rem_limit <= 0:
                state = 'booked'
            rec.state = state
    ...
    rem_limit = fields.Integer(compute="get_acs_remaining_limit",string='Remaining Limit',store=True)
    acs_slot_id = fields.Many2one('acs.schedule.slot', string="Slot", ondelete="cascade")
    acs_schedule_id = fields.Many2one("acs.schedule", related="acs_slot_id.acs_schedule_id", string="Booking schedule", store=True)
    company_id = fields.Many2one('res.company', string='Company', related="acs_slot_id.acs_schedule_id.company_id", store=True)
    
    schedule_type = fields.Selection([('service','Service')], string="Schedule Type", default="service")
    price = fields.Float(compute="acs_get_booking_price", string="Appointment Fees")
    state = fields.Selection([
        ('available', 'Available'),
        ('booked', 'Booked'),
        ('blocked', 'Blocked'),
        ('cancel', 'Cancelled')        
    ], compute="get_acs_status", string="Status", default='available', store=True)
    is_blocked = fields.Boolean(string="Is Blocked", default=False)
    blocking_user_id = fields.Many2one('res.users', string="Blocked By")
    ...
    #Hook Method to implement in other module.
